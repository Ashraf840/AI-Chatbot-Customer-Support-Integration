...
class ChatRoomCreateAPISocket(APIView):
    ...
    def post(self, request, format=None):
        if 'user_email' in request.data and 'chatbot_socket_id' in request.data:
            ...
            # TODO; Required code refactoring; Place the common variables in a general place
            ...
            try:
                if bool(request.data['prompt_user']):
                    print("request.data['prompt_user']:", request.data['prompt_user'])
                    print("request.data['prompt_user'] - type:", type(request.data['prompt_user']))
                ...
                print("TMS Issuer Oid (ChatRoomCreateAPISocket API):", issuerOid)
                user_chatbot_socket = UserChatbotSocket.objects.filter(chatbot_socket_id=chatbot_socket_id)
                ...
                if len(user_chatbot_socket) == 1:
                    ...
                    print(f"Length of active_cso (chat_room_create_api): {len(active_cso)}")
                    if len(active_cso) == 0:
                        return Response("No CSO is currently available!")
                    if len(active_cso) == 1:
                        print(f'Active cso (ChatRoomCreateAPISocket - API): {active_cso[0]["cso_email"]}')
                        print(f'Active cso (ChatRoomCreateAPISocket - API): {active_cso}')
                        total_msg = CustomerSupportRequest.get_reqs_with_assigned_cso(cso_email=active_cso[0]["cso_email"])
                        if len(total_msg) >= 5:
                            print(f'The HDO {active_cso[0]["cso_email"]} is occupied with max support-request')
                            return Response("No CSO is currently available!")
                        else:
                            print(f'Sending a message request to {active_cso[0]["cso_email"]}')
                            channel_layer = get_channel_layer()
                            email_normalized="".join(ch for ch in user_email if ch.isalnum())
                            async_to_sync(channel_layer.group_send)(
                                f'user_chatbot_socket_{email_normalized}',
