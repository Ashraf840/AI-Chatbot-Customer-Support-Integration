...
def transcribe(request):
    ...
    file_path = f'./media/audio/{t}-blob.webm'  # Provide the desired file path
    with open(file_path, 'wb') as file:
        file.write(content_file.read())

    r = sr.Recognizer()

    webm_file = f'./media/audio/{t}-blob.webm'
    wav_file = f"./media/audio/{t}-blob.wav"
    command = ['ffmpeg', '-i', webm_file, wav_file]
    subprocess.run(command, check=True)

    # Transcribe the audio
    r = sr.Recognizer()
    audio = sr.AudioFile(f"./media/audio/{t}-blob.wav")
    text = ""
    try:
        with audio as source:
            ...
            if detect_language(wav_file) == "bn":    
                ...
                # Make API call for speech-to-text
                api_url = 'https://nlp.celloscope.net/nlp/speech-to-text/v2'
                files = {'files': open(wav_file, 'rb')}
                data = {'format': 'wav'}
                ...
                if response.status_code == 200:
                    ...
                    print("It is from celloscope stt")
                else:
