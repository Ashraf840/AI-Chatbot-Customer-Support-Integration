...
def createCSOTMS(email, username, userNameBn, userId, initial_password, mobileNumber):
    ...
    # Create call-center-agent using the user's ("chat.operator.system.admin") credentials
...
def user_signal(sender, instance, created, **kwargs):
    if created:
        ...
        if instance.is_cso:
            ...
            # Create a call-center agent inside TMS (using a fixed admin-user of TMS - "ChatOperatorSystemAdmin")
            userName = instance.first_name + ' ' + instance.last_name
            # user_profile = User_Profile.objects.get(user_email=instance.email)
